...
describe 'Edit' do
  context 'unchanged split points' do
    before :each do
      @tag_list = "genre,year,artist" 
      @hash_version_of_data = {'file_name' => "180919_0688.MP3", 'discard_before' => "0", 'discard_after' => "0", 'artist' => "Martin Pickersgill", 'album' => "Alberti", 'song' => "BWV847", 'genre' => "32", 'TYER' => "2018", 'TIT2' => "Martin Pickersgill, J S Bach, Prelude in C Minor, Bk 1, Alberti, 19 Sept 2018"}
      @array_version_of_data = [["file_name", "discard_before", "discard_after", "artist", "album", "song", "genre", "TYER", "TIT2"],["180919_0688.MP3", "0", "0", "Martin Pickersgill", "Alberti", "BWV847", "32", "2018", "Martin Pickersgill, J S Bach, Prelude in C Minor, Bk 1, Alberti, 19 Sept 2018"]]
    ...
    xit 'predicts an unchanged filename' do
      expect(@edit_predict.wildcard_filename).to eq('180919_0688.MP3')
    end
  end
  context 'EOF instead of zero' do
    ...
    it 'predicts the filename resulting from the split' do
      expect(@edit_predict.wildcard_filename).to eq('180919_0688_00m_53s__*.mp3')
    end
